...
5. Slow Provisioning: Provisioning new resources or environments was a time-consuming process that involved multiple manual steps, leading to delays in project delivery. Let say we've been asked to create S3 buckets for multiple teams altogether, so if we do it using management console, it will take a lot of time instead we can do it by using IAC efficiently.
