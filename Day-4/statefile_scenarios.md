...
1. **Security Risks**: Sensitive information, such as API keys or passwords, may be stored in the state file if it's committed to a VCS (Version Control System). This poses a security risk because VCS repositories are often shared among team members.
...
Let say one fo the person in your team cloned the repo, did some changes tot eh infra and then did terraform apply, so there will be 2 changes: one to the main.tf (or any other config file) and terraform.tfstate. Now he will need to commit both the files. If he commits only the main.tf, next time if someone pulls the changes and then does terraform plan, he will be shown that resources as pending changes and need to apply that to be recorded in terraform.tfstate.
